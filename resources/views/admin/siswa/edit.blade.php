  ...
  <form action="{{ route('admin.siswa.update', $student->id) }}" method="post">
    ...
    <label class="block mt-4 text-sm">
        ...
        <input
          ...
          placeholder="11806858" name="nisn" value="{{ $student->nisn ?? old('nisn') }}" required
        />
        @error('nisn')
          <span>
    ...
    <label class="block mt-4 text-sm">
        ...
        >
          @foreach ($classrooms as $classroom)
            <option {{ $classroom->id == $student->classroom_id ? 'selected' : '' }} value="{{ $classroom->id }}">{{ $classroom->name }}</option>
          @endforeach
        </select>
        @error('classroom_id')
          <span>
    ...
    <label class="block mt-4 text-sm">
      ...
      <input
        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
        placeholder="nama depan..." name="first_name" value="{{ $student->first_name ?? old('first_name') }}" required
      />
      @error('first_name')
        <span>
            <strong style="color: red;">{{ $message }}</strong>
    ...
    <label class="block mt-4 text-sm">
        ...
        <input
          class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
          placeholder="nama belakang..." name="last_name" value="{{ $student->last_name ?? old('last_name') }}" required
        />
        @error('last_name')
          <span>
              <strong style="color: red;">{{ $message }}</strong>
    ...
    <label class="block mt-4 text-sm">
        ...
        <input
          class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
          type="date" name="date_of_birth" value="{{ $student->date_of_birth }}" required
        />
        @error('date_of_birth')
          <span>
              <strong style="color: red;">{{ $message }}</strong>
